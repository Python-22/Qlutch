...
public class createTC extends CommandManager {
		...
		if (plugin.registeredPremium.contains(p.getName())) {
			if (plugin.isLoggedIn) {
				...
				if (guild == null) {
					...
					if (args.length <= 1) {
						guild.getController().createTextChannel(Settings.AUTHOR).queue();
					} else {
						guild.getController().createTextChannel(args[1]).queue();
					}
